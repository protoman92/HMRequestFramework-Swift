...
public struct NavigationViewModel {
  ...
  public func errorStream() -> Observable<Try<Error>> {
    let path = GeneralReduxAction.Error.Display.errorPath
    return provider.reduxStore.stateValueStream(Error.self, path)
  }

  public func progressStream() -> Observable<Try<Bool>> {
    let path = GeneralReduxAction.Progress.Display.progressPath
    return provider.reduxStore.stateValueStream(Bool.self, path)
  }

  public func clearError() {
    let store = provider.reduxStore
    let action = GeneralReduxAction.Error.Display.updateShowError(nil)
